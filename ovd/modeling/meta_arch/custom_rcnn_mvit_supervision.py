...
class CustomRCNNMViT(GeneralizedRCNN):
    ...
    def from_config(cls, cfg):
        ...
        ret.update({
            ...
            'fp16': cfg.FP16,
            'freeze_image_backbone': cfg.MODEL.FREEZE_IMAGE_BACKBONE,
        })
        return ret
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.fp16:
            ...
            features = self.backbone(images.tensor)
        if self.training and self.freeze_image_backbone and ann_type == 'image':
            features = {k: v.detach() for k, v in features.items()}

        rpn_targets = gt_instances if ann_type == 'box' else None
        rpn_proposals, proposal_losses = self.proposal_generator(
            images, features, rpn_targets)

        # Paper-style weak supervision: treat RPN proposals as the MIL bag for image-level data.
        # Keep support for externally provided class-specific pseudo proposals if present.
        proposals = rpn_proposals
        if (self.with_image_labels) & (ann_type == 'image'):
            if len(batched_inputs) > 0 and 'cls_specific_target_props' in batched_inputs[0]:
    ...
    def convert_output_rpn_format_target(self, batched_inputs, images):
        ...
        for n, image_size in enumerate(images.image_sizes):
            ...
            if 'cls_specific_target_props' not in batched_inputs[n]:
                ...
                target_props = [(int(key), box, 1.0)]
            else:
                probas = batched_inputs[n]['cls_specific_scores']
                boxes = batched_inputs[n]['cls_specific_props']
                target_props = batched_inputs[n]['cls_specific_target_props']
            keep = (probas > conf_thresh)
            keep_list = keep.to(torch.bool).cpu().tolist()
            boxes_kept = boxes[keep].to(images.device)
            probas_kept = probas[keep].to(images.device)
            target_props_kept = [tp for tp, k in zip(target_props, keep_list) if k]
            res.proposal_boxes = Boxes(boxes_kept)
            res.objectness_logits = probas_kept
            res.target_proposals = target_props_kept
            res.target_classes = torch.tensor(
                [tp[0] for tp in target_props_kept], device=images.device, dtype=torch.int64
            ) if len(target_props_kept) else torch.empty((0,), device=images.device, dtype=torch.int64)
